...
def dflash_generate(
    ...
    acceptance_lengths = []
    token_ranks = [] if return_topk_recall else None
    ...
    while start < max_length:
        ...
        if return_topk_recall and block_size > 1:
            # Score every position in the block. j < acceptance_length is the
            # accepted prefix (rank is trivially 0, since accept means draft
            # top-1 == posterior); j == acceptance_length is the rejected/
            # bonus position; j > acceptance_length was never reached by the
            # actual accept/reject walk. Rejection only means top-1 was wrong
            # at that position, not that the whole predicted distribution
            # was bad -- what matters is just whether the target's own
            # per-position judgment (posterior[j], computed against the
            # as-drafted block, same as the verifier already uses) falls
            # inside the drafter's top-k there. One round contributes
            # exactly block_size-1 ranks, in position order, so analysis can
            # split "pre-reject" (index < this round's accept_lengths entry)
            # from "reject and beyond".
            true_tokens = posterior[0, : block_size - 1]
            true_logits = draft_logits[0].gather(1, true_tokens.unsqueeze(1)).squeeze(1)
            ranks = (draft_logits[0] > true_logits.unsqueeze(1)).sum(dim=1)
            token_ranks.extend(ranks.tolist())
